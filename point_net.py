...
class PointNetAutoEncoder(nn.Module):
    def __init__(self, num_points , num_global_feats):
        ...
        # Decoder
        self.linear1 = nn.Linear(num_global_feats, 1024)
        self.linear2 = nn.Linear(1024, 1024)
        self.linear3 = nn.Linear(1024, num_points*3)
    ...
    def forward(self, x):
        ...
        # FC Decoder
        x = self.bn6(F.relu(self.linear1(global_features)))
        x = self.bn7(F.relu(self.linear2(x)))
        x = self.linear3(x)
        x = x.view(bs, 3, self.num_points)
        
        return x, global_features, critical_indexes 
# ============================================================================
